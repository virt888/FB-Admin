...
class CameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, ItemSelectionViewControllerDelegate {
	...
	override func viewDidLoad() {
		super.viewDidLoad()
        title = "Scan boaring pass" 
		let historyBtn = UIButton(frame: CGRect(x: 0, y: 0, width: 80, height: 30))
        historyBtn.setTitle("History", for: .normal)
        historyBtn.titleLabel?.font = UIFont(name: "PingFang HK", size: 16)
        historyBtn.addTarget(self, action: #selector(gotoHistoryTable(sender:)), for: .touchUpInside)
        
        let barHistoryBt = UIBarButtonItem(customView: historyBtn)
        navigationItem.rightBarButtonItems = [barHistoryBt]
	...
	*/
	...
	func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
		...
		if metadataObjectsOverlayLayersDrawingSemaphore.wait(timeout: DispatchTime.now()) == .success {
			DispatchQueue.main.async { [unowned self] in
				self.removeMetadataObjectOverlayLayers()
                if let stringValue = metadataObjects.first as? AVMetadataMachineReadableCodeObject {
//                    let resultVC = ResultViewController()
//                    resultVC.resultLab.text = stringValue.stringValue
//                    self.navigationController?.pushViewController(resultVC, animated: false)
                    //var resultInfo = ""
//                    let record = ScanRecord(barcode: stringValue.stringValue, fbName: "Jake", amout: "300", userName: "Mona", updateTime: Date().description)
//                    storeRecord(record: record, success: { 
//                        resultInfo = "Insert a record successfully!"
//                    }, faile: { (error) in
//                        resultInfo = error
//                    })
//                    let alertController = UIAlertController(title: "Title", message: resultInfo, preferredStyle: .alert)
//                    let okAction = UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil)
//                    alertController.addAction(okAction)
                    let checkVC = CheckPassengerViewViewController()
                    self.navigationController?.pushViewController(checkVC, animated: false)
                    
                    self.session.stopRunning()
                }else if let stringValue = metadataObjects.first as? AVMetadataObject {
                    let resultVC = ResultViewController()
                    resultVC.resultLab.text = stringValue.description
                    self.navigationController?.pushViewController(resultVC, animated: false)
                    self.session.stopRunning()
